...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.AuthModal = __webpack_require__(170);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var registerForm = __webpack_require__(197);
	var forgotForm = __webpack_require__(198);
	...
	AuthModal.prototype.submitLoginForm = function (form) {
	  ...
	  request.addEventListener('success', function (event) {
	    ...
	    if (request.status == 401) {
	      onEnd();
	      _this.onAuthFailure(event.result.message);
	      return;
	    }
	
	    // don't stop progress indication if login successful && we're making redirect
	    if (!_this.options.callback) {
	      _this.onAuthSuccess(event.result.user);
	    } else {
	      onEnd();
	      _this.onAuthSuccess(event.result.user);
	    }
	  ...
	  request.addEventListener('fail', function (event) {
	    onEnd();
	    _this.onAuthFailure(event.reason);
	  });
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	jade_mixins["e"].call({
	block: function(){
	buf.push("Авторизация работает в тестовом режиме. О любых проблемах и странностях сообщайте, пожалуйста, на <a href=\"https://github.com/iliakan/javascript-nodejs/issues/new\">github</a>.");
	},
	attributes: {"class": "info"}
	});
	},
	attributes: {"class": "line __notification"}
	});
	jade_mixins["e"].call({
	...
	attributes: {"for": "auth-email","class": "label"}
	}, 'label');
	jade_mixins["b"].call({
	block: function(){
	jade_mixins["e"].call({
	attributes: {"id": "auth-email","name": "email","type": "email","autofocus": true,"class": "control"}
	}, 'input');
	},
	...
	attributes: {"for": "auth-password","class": "label"}
	}, 'label');
	jade_mixins["b"].call({
	block: function(){
	jade_mixins["e"].call({
	attributes: {"id": "auth-password","type": "password","name": "password","class": "control"}
	}, 'input');
	jade_mixins["e"].call({
	block: function(){
	buf.push("Забыли?");
	...
	attributes: {"class": "close-link tablet-only modal__close"}
	}, 'a');
	},
...
/***/ function(module, exports, __webpack_require__) {
	...
	jade_mixins["e"].call({
	block: function(){
	buf.push("Регистрация работает в тестовом режиме. О любых проблемах и странностях сообщайте, пожалуйста, на <a href=\"https://github.com/iliakan/javascript-nodejs/issues/new\">github</a>.");
	},
	attributes: {"class": "info"}
	});
	},
	...
	jade_mixins["e"].call({
	block: function(){
	buf.push("Регистрируясь, вы принимаете <a href=\"/agreement\">пользовательское соглашение</a>.");
	},
	attributes: {"class": "line __agreement"}
	});
	jade_mixins["e"].call({
	block: function(){
	jade_mixins["e"].call({
...
//# sourceMappingURL=authClient-7.e45a72e72551cd8f9ecb.js.map